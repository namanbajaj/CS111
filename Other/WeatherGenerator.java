...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        int[] day = new int[numberOfDaysInMonth[month - 2]];
        ...
        for(int i = 1; i < numberOfDaysInMonth[month - 2]; i++){
            if(day[i - 1] == WET){
                double randTest = StdRandom.uniform();
                for(int k = 0; k < wetwet.length - 1; k++){
                    if(wetwet[k][0] == longitute && wetwet[k][1] == latitude){
                        double wetToWet = wetwet[k][month];
                        if(randTest < wetToWet)
                            day[i] = WET;
                        else
                            day[i] = DRY;
                        break;
                ...
                double randTest = StdRandom.uniform();
                for(int k = 0; k < drywet.length - 1; k++){
                    if(drywet[k][0] == longitute && drywet[k][1] == latitude){
                        double dryToWet = drywet[k][month];
                        if(randTest < dryToWet)
                            day[i] = WET;
                        else
                            day[i] = DRY;
                        break;
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        ...
        for(int i = 0; i <= forecast.length - 1; i++){
            if(forecast[i] == mode){
                ...
                while(i != forecast.length - 1 && forecast[i] == forecast[i + 1]){
                    tempCount++;
                    i++;
                }
